...
const StatCard = ({ title, value, icon: Icon, helper, barPercent }) => {
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#dbeafe] p-4 shadow-sm flex flex-col gap-1">
      ...
      <div className="flex items-center justify-between">
        ...
        {Icon && <Icon className="w-7 h-7 text-[#0b63c7]" />}
      </div>
      ...
      {percent !== null && (
        <div className="mt-2 w-20 sm:w-24 h-2 rounded-full bg-[#e5e7eb] overflow-hidden">
          <div
            className="h-full bg-[#0b63c7] transition-all"
            style={{ width: `${percent}%` }}
          />
        </div>
      )}
